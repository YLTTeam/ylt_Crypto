...
- (void)viewDidLoad
{
    ...
    NSLog(@"%@", data);
    NSData *result = [YLT_DESCrypto encryptData:data keyString:@"ylt" iv:nil];
    NSLog(@"des(ECB) 加密后字串: %@", result);
    result = [YLT_DESCrypto dencryptData:result keyString:@"ylt" iv:nil];
    NSLog(@"des(ECB) 解密后字串: %@", result);
    
    result = [YLT_DESCrypto encryptData:data keyString:@"ylt" iv:ivData];
    NSLog(@"des(CBC) 加密后字串: %@", result);
    result = [YLT_DESCrypto dencryptData:result keyString:@"ylt" iv:ivData];
    NSLog(@"des(CBC) 解密后字串: %@", result);
    
    
    result = [YLT_AESCrypto encryptData:data keyString:@"ylt" iv:nil];
    NSLog(@"aes(ECB) 加密后字串: %@", result);
    result = [YLT_AESCrypto dencryptData:result keyString:@"ylt" iv:nil];
    NSLog(@"aes(ECB) 解密后字串: %@", result);
    
    result = [YLT_AESCrypto encryptData:data keyString:@"ylt" iv:ivData];
    NSLog(@"aes(CBC) 加密后字串: %@", result);
    result = [YLT_AESCrypto dencryptData:result keyString:@"ylt" iv:ivData];
